...
// ── FIX: shared clamp helper — used anywhere a raw score/probability
// is rendered with a literal "%" suffix, so a value from any backend
// path can never render above 100 or below 0 in the UI.
  ...
  return (
    <div className="relative w-24 h-24 flex items-center justify-center">
      <svg width="88" height="88" viewBox="0 0 88 88" className="-rotate-90">
        <circle cx={c} cy={c} r={r} fill="none" stroke="#e5e7eb" strokeWidth="7" />
        <circle cx={c} cy={c} r={r} fill="none"
          stroke={scoreRing(clamped)} strokeWidth="7"
          strokeDasharray={`${dash} ${circ}`}
          strokeLinecap="round"
          style={{ transition: "stroke-dasharray 1s ease" }}
        />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`text-2xl font-black ${scoreColor(clamped)}`}>{clamped}</span>
        <span className="text-[10px] text-gray-400 -mt-0.5">/ 100</span>
      </div>
    </div>
  );
...
// ── Notice Gauge ──────────────────────────────────────────────
function NoticeGauge({ probability, riskLevel }: { probability: number; riskLevel: string }) {
  const clamped = clampPct(probability);
  const colors = NOTICE_COLORS[riskLevel] || NOTICE_COLORS.MEDIUM;
  return (
    <div>
      <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
      ...
      <div className="flex justify-between mt-1">
        <span className="text-[10px] text-gray-400">0%</span>
        <span className={`text-[10px] font-bold ${colors.text}`}>{clamped}%</span>
        <span className="text-[10px] text-gray-400">100%</span>
      </div>
    </div>
  );
...
function NoticeSimulatorSection({ sim, lang }: { sim: NoticeSimulation; lang: Lang }) {
  ...
  const colors      = NOTICE_COLORS[sim.risk_level] || NOTICE_COLORS.MEDIUM;
  const whatIf      = sim.what_if_fix_all;
  const probability = clampPct(sim.probability);
    ...
    <div className={`rounded-2xl border overflow-hidden ring-2 ${colors.ring} shadow-lg ${colors.glow} mb-5`}>
      <div className={`px-4 lg:px-6 py-4 lg:py-5 ${colors.bg}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
              probability >= 50 ? "bg-red-100" : probability >= 25 ? "bg-orange-100" : "bg-green-100"
            }`}>
              <AlertTriangle size={20} className={
                probability >= 50 ? "text-red-600" : probability >= 25 ? "text-orange-600" : "text-green-600"
              } />
            </div>
            <div>
              <h2 className="font-bold text-gray-900 text-base">
          ...
          <div className={`text-3xl lg:text-4xl font-black shrink-0 ${colors.text}`}>{probability}%</div>
        </div>
        <div className="mt-3">
          <NoticeGauge probability={probability} riskLevel={sim.risk_level} />
        </div>
      </div>
      ...
      <div className="px-4 lg:px-6 py-4 lg:py-5 space-y-5 bg-white">
        {sim.risk_areas?.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {sim.risk_areas.slice(0, 5).map((area, i) => {
                // ── FIX: clamp before display — belt-and-braces alongside
                // the backend cap in notice_simulator.py, so the UI can
                // never show an impossible >100% even if some other
                // code path feeds this component an uncapped score.
                const displayScore = clampPct(area.score);
                return (
                  <div key={i} className="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-lg">
                    <div className={`w-1.5 h-8 rounded-full shrink-0 ${
                      area.dept_priority === "VERY HIGH" ? "bg-red-500" :
        ...
        {whatIf && (
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
            ...
            <div className="flex items-center gap-3">
              <div className="text-center">
                <p className="text-2xl font-black text-red-600">{clampPct(whatIf.current_probability)}%</p>
                <p className="text-[10px] text-gray-500">{lang === "hi" ? "अभी" : lang === "mr" ? "सध्या" : "Now"}</p>
              </div>
              <div className="text-green-500 font-bold text-lg">→</div>
              <div className="text-center">
                <p className="text-2xl font-black text-green-600">{clampPct(whatIf.new_probability)}%</p>
                <p className="text-[10px] text-gray-500">{lang === "hi" ? "बाद में" : lang === "mr" ? "नंतर" : "After"}</p>
              </div>
              <div className="ml-auto bg-green-600 text-white px-3 py-2 rounded-xl text-center">
                <p className="text-base font-black leading-none">-{whatIf.reduction}%</p>
